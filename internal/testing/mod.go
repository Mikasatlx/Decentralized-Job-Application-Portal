...
// WithStorage sets a specific storage
func WithHrStorage(hrStorage storage.HrStorage) Option {
	return func(ct *configTemplate) {
		ct.hrStorage = hrStorage
	}
}
...
func NewTestNode(t require.TestingT, f peer.Factory, trans transport.Transport,
	...
	config.HrStorage = template.hrStorage
...
// GetHrStorage returns the storage provided to the node.
func (t TestNode) GetHrStorage() storage.HrStorage {
	return t.config.HrStorage
}
...
// DisplayBlokchainBlocks writes a string representation of all blocks store in
// the storage.
func DisplayPbftchainBlocks(t *testing.T, out io.Writer, IDhr string, hrStore storage.HrStore) {

	fmt.Println("start a new block chain \n")

	lastBlockHashHex := hex.EncodeToString(hrStore.Get(IDhr, storage.LastBlockKey))
	endBlockHasHex := hex.EncodeToString(make([]byte, 32))

	for lastBlockHashHex != endBlockHasHex {
		lastBlockBuf := hrStore.Get(IDhr, string(lastBlockHashHex))
		...
		lastBlock.DisplayPbftBlock(out)

		lastBlockHashHex = hex.EncodeToString(lastBlock.PrevHash)
	}

	fmt.Println("this block chain ends\n")
}
