...
const main = function () {
    ...
    // project
    application.register("blockchain", Blockchain)
...
// project we need to show blockchain
class Blockchain extends BaseElement {
    static get targets() {
        return ["password"];
    }
    ...
    async get() {
        try {
            const ok = this.checkInputs(this.passwordTarget);
            if (!ok) {
                return;
            }
            const password = this.passwordTarget.value
            const addr = this.peerInfo.getAPIURL("/blockchain");
            const resp = await this.fetch(addr)
            const text = await resp.text();
            let myWindow=window.open('','');
            myWindow.document.write(text);
            myWindow.focus();
            this.flash.printSuccess("get blockchain!, password:" + password);
        } catch (e) {
            this.flash.printError("failed: " + e);
        }
    }
