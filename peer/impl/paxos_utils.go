...
func (n *node) ResponsePrepare(prepare *types.PaxosPrepareMessage, val types.PaxosValue, id uint) error {
	...
	transPromise, err := n.MessageRegistry.MarshalMessage(&promise)
	if err != nil {
		return err
	}
	...
	transPrivate, err := n.MessageRegistry.MarshalMessage(&private)
	if err != nil {
		return err
	}
	go func() {
...
func (n *node) ResponsePropose(propose *types.PaxosProposeMessage) error {
	...
	// 2. broadcast it
	transAccept, err := n.MessageRegistry.MarshalMessage(&accept)
	if err != nil {
		return err
	}
	go func() {
...
func (n *node) ProposePhase1(round uint, curStep uint) (types.PaxosValue, bool, error) {
	...
	// 2. broadcast it
	transPrepare, err := n.MessageRegistry.MarshalMessage(&prepare)
	if err != nil {
...
func (n *node) ProposePhase2(round uint, val types.PaxosValue, curStep uint) (bool, error) {
	...
	// 2. broadcast it
	transPropose, err := n.MessageRegistry.MarshalMessage(&propose)
	if err != nil {
		return false, err
	}
	go func() {
...
func (n *node) GetBlock(consensus types.PaxosValue, step uint) types.BlockchainBlock {
	prevHash := n.Storage.GetBlockchainStore().Get(storage.LastBlockKey)
	if prevHash == nil {
...
func (n *node) BroadcastTLC(step uint, block types.BlockchainBlock) error {
	...
	transTLCMsg, err := n.MessageRegistry.MarshalMessage(TLCMsg)
	if err != nil {
		return err
	}
	go func() {
...
func (n *node) TLCProcess(step uint, catchUp bool) error {
	...
	// 2. update namestore
	n.Storage.GetNamingStore().Set(block.Value.Filename, []byte(block.Value.Metahash))
