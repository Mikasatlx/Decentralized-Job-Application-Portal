...
func Test_HW3_Integration_Multiple_Consensus(t *testing.T) {
	...
	for i, node := range nodes {
		...
		// for project! both student nodes and reference nodes are student nodes
		consensus := node.GetBlockchain()
		// get paxosValList for referen
		var init bool
		if paxosValList == nil {
			paxosValList = make([]string, len(consensus))
			init = true
		}
		// print and test if they are the same
		consensusStr := ""
		for i, c := range consensus {
			...
			if init {
				...
				require.Equal(t, paxosValList[i], cStr)
			}
		}
